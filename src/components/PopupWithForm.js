    ...
    return (
        <section className={`popup popup_${name} ${isOpenPopup}`}>
            <div className="popup__container">
                <button className="popup__close popup__close_delete" aria-label="Закрыть" onClick = {onClose} />
                <form className="popup__content" onSubmit={onSubmit} name={`${name}`}>
                    <h2 className="popup__title">{title}</h2>
                    {children}
                </form>
                <button type="submit" className="popup__button-submit" onClick={onSubmit}>{buttonText}</button>
            </div>
        </section>
    )
}
